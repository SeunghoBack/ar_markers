...
def main(argv, imgName):
	...
	for marker in markers:
		...
		if minY < 20 or minX < 20 or maxY > frame.shape[0]-20 or maxX > frame.shape[1]-20:
			continue
		imgO = frame[minY-20:maxY+20, minX-20:maxX+20]
		img = cv2.resize(imgO, dsize=(500,500), interpolation=cv2.INTER_AREA)
	...
	if len(meanList) != 0:
		...
		totalMeans.append(sum(meanList)/len(meanList))
		totalMaxs.append(max(meanList))
		totalMins.append(min(meanList))
		RecognizedImage+=1
...
if __name__ == '__main__':
	...
	for imgName in imgNames:
		if RecognizedImage > 1:
			fa = open(fPath+'/result/total_info.txt', 'w', newline='')
			data = "Total Mean : %f\n" %(sum(totalMeans)/len(totalMeans))
			fa.write(data)
			data = "Total Std : %f \n" %statistics.stdev(totalMeans)
			fa.write(data)
			data = "Total Images : %d \n" %len(imgNames)
			fa.write(data)
			data = "Recognized Image : %d" %RecognizedImage
			fa.write(data)
			fa.close()
